...
- **Gitignored:** `antarctica/mesh/*` (meshes and all MAP/inversion `.h5`),
  `antarctica/results/` (timeseries CSVs, checkpoints, logs), `ISMIP7/` and
  `antarctica/data/` (forcing and observational data), all `*.h5`.
- **Therefore:** `antarctica/reports/coreNN_*.md` is the ONLY committed record
  of a run. It carries the env knobs, the mass budget at marker years and the
  observational audit verdict. Generated by `antarctica/scripts/core_report.py`.
