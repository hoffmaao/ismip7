...
| `ISMIP7_OUTPUT` | `1`/`on` to record the ISMIP7 yearly fields and scalars alongside the run (`<exp>_<lc>_ismip7_annual.h5` and `<exp>_<lc>_ismip7_scalars.csv`, regridded afterwards by `scripts/write_ismip7_output.py`); `0`/`off`/unset disables it. The value set is closed, so a typo is rejected at startup rather than silently enabling the output. A chained projection that is to be submitted must export it on every link | _(unset, off)_ |
