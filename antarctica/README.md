...
| `ISMIP7_WALL_STOP_MIN` | wall-clock budget in minutes, counted from process start. On reaching it the run finishes the current step, writes its final checkpoint and exits cleanly with `t_yr` short of `t_end`, which is what a chained batch job resumes from. Without it a job that hits its scheduler limit is killed mid-step and loses everything since the last periodic checkpoint. `nots_projection.sbatch` derives it from the job's own `TimeLimit` less `ISMIP7_WALL_MARGIN_MIN` (default 25). `0` disables the budget | `0` |
