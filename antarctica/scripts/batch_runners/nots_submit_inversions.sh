...
REPO=$PWD
which=${1:-BC}
COMMON="ISMIP7_LC=2000,ISMIP7_BUFFER_M=0,ISMIP7_DHDT_NET_SIGMA=10,ISMIP7_LOG_VEL_WEIGHT=auto"
if [[ "$which" == *C* ]]; then
  sbatch -p long -C sapphirerapids --mem=240G -N1 -n32 --cpus-per-task=1 --time=3-00:00:00 -J inv2k_int5k \
    --export=ALL,$COMMON,ISMIP7_LC_COARSE=5000,ISMIP7_MESH=$REPO/antarctica/mesh/antarctica_5000_2000_buffered0.msh,ISMIP7_RASTER_SAMPLE=vertex,ISMIP7_MAP_OUT=$REPO/antarctica/mesh/inversion_icepack2_budd_n3_dg0_logvelnet_2000_int5000.h5 \
...
if [[ "$which" == *B* ]]; then
  sbatch -p long -C sapphirerapids --mem=200G -N1 -n32 --cpus-per-task=1 --time=3-00:00:00 -J inv2k_cellmean \
    --export=ALL,$COMMON,ISMIP7_LC_COARSE=20000,ISMIP7_MESH=$REPO/antarctica/mesh/antarctica_20000_2000_buffered0.msh,ISMIP7_RASTER_SAMPLE=cell_mean,ISMIP7_MAP_OUT=$REPO/antarctica/mesh/inversion_icepack2_budd_n3_dg0_logvelnet_2000_cellmean.h5 \
    antarctica/scripts/batch_runners/nots_inversion.sbatch
fi
