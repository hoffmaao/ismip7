...
flotation test and driving stress use, and ``height_above_flotation`` is the
one flotation test every law here and the external-law state read. The CalvingMIP protocol and the
...
from firedrake import Constant, conditional, dot, lt, max_value, min_value, sqrt
...
def thickness_calving_rate(u, h, b, h_critical):
    r"""``c = max(0, 1 + (Hc - H) / Hc) |u|`` [m/yr along the front normal]
    where the bed is below sea level, and zero elsewhere.
    ...
    a 152.4 m floating-front median.

    The level set anchors on every ice/no-ice facet, so without a gate the
    rule would erode land-terminating margins and nunataks thinner than
    ``Hc`` and book the loss as calving. The gate is the bed below sea level,
    so a marine grounded cliff still calves.
    """
    thickness = max_value(h, Constant(0.0))
    hc = Constant(float(h_critical))
    factor = max_value(Constant(1.0) + (hc - thickness) / hc, Constant(0.0))
    speed = sqrt(dot(u, u) + Constant(1e-30))
    marine = conditional(lt(b, Constant(0.0)), Constant(1.0), Constant(0.0))
    return factor * speed * marine
