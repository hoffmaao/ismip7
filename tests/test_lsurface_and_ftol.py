...
def test_the_corner_is_where_two_power_laws_meet():
    """A misfit that flattens once gamma is small enough and a prior energy
    that keeps growing with gamma meet at gamma = 1: that joint is the point
    of maximum curvature, whichever way the points are ordered."""
    ls = _lsurface()
    gammas = np.logspace(-3, 3, 13)
    misfit = 1.0 + 1.0 / gammas ** 2          # falls steeply, then flat
    reg = 1.0 + gammas ** 2                    # flat, then rises steeply
